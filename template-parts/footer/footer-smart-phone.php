<footer class="footer-smart">
  <a href="<?php echo SITE_URL?>catalog" class="footer-smart__item footer-smart__item_catalog">
    <svg class="footer-smart__icon"  fill="#164CB5" width="26" height="24" viewBox="0 0 26 24">
      <path d="M17.857 11.143c.238 0 .429.191.429.428v7.715c0 .237-.191.428-.429.428-.237 0-.428-.19-.428-.428V11.57c0-.237.19-.428.428-.428zm-5.143 0c.238 0 .429.191.429.428v7.715c0 .237-.191.428-.429.428-.237 0-.428-.19-.428-.428V11.57c0-.237.19-.428.428-.428zm-5.143 0c.238 0 .429.191.429.428v7.715c0 .237-.191.428-.429.428-.237 0-.428-.19-.428-.428V11.57c0-.237.191-.428.428-.428zM16.165 0c.112 0 .217.05.294.129l5.134 5.142c.38.382-.222.99-.606.607L15.853.735C15.577.463 15.73 0 16.165 0zM9.263 0c-.111 0-.216.05-.294.129L3.835 5.27c-.379.382.223.99.606.607L9.575.735C9.853.463 9.701 0 9.265 0h-.002zM1.571 6.857c-.354 0-.683.137-.93.37-.249.234-.417.6-.348.99l2.571 14.571c.053.3.174.591.388.823.215.23.537.389.892.389h17.143c.354 0 .677-.16.891-.39.214-.231.334-.521.387-.823l2.572-14.571c.068-.389-.1-.755-.348-.989-.25-.233-.577-.37-.932-.37H1.571zm0 .857h22.286c.429 0 .484.176.436.355L21.72 22.641c-.103.38-.197.502-.435.502H4.143c-.313 0-.352-.189-.436-.502L1.136 8.069c-.053-.283.221-.355.435-.355z"/>
    </svg>
    <p class="footer-smart__text">Каталог</p>
  </a>
  <a href="<?php echo SITE_URL?>discounts" class="footer-smart__item footer-smart__item_sales">
    <svg class="footer-smart__icon" width="24" height="24" viewBox="0 0 24 24">
      <path d="M3 11.5c0 .276-.224.5-.5.5s-.5-.224-.5-.5.224-.5.5-.5.5.224.5.5zm9 9c0 .276-.224.5-.5.5s-.5-.224-.5-.5.224-.5.5-.5.5.224.5.5zm9-9c0 .276-.224.5-.5.5s-.5-.224-.5-.5.224-.5.5-.5.5.224.5.5zm-9-9c0 .276-.224.5-.5.5s-.5-.224-.5-.5.224-.5.5-.5.5.224.5.5zM11.5 4c-.277 0-.5.223-.5.5v5.594c-.58.208-1 .76-1 1.406 0 .823.677 1.5 1.5 1.5.647 0 1.198-.42 1.406-1H16.5c.277 0 .5-.223.5-.5s-.223-.5-.5-.5h-3.594c-.152-.422-.484-.754-.906-.906V4.5c0-.277-.223-.5-.5-.5zm0 7c.282 0 .5.218.5.5 0 .282-.218.5-.5.5-.282 0-.5-.218-.5-.5 0-.282.218-.5.5-.5zm0-11C5.155 0 0 5.155 0 11.5S5.155 23 11.5 23 23 17.845 23 11.5 17.845 0 11.5 0zm0 1C17.305 1 22 5.695 22 11.5S17.305 22 11.5 22 1 17.305 1 11.5 5.695 1 11.5 1z"/>
    </svg>
    <p class="footer-smart__text">Скидки</p>
  </a>
  <a href="#" class="footer-smart__item footer-smart__item_help">
    <svg class="footer-smart__icon" width="24" height="24" viewBox="0 0 24 24">
      <path d="M15.5 8c.277 0 .5.223.5.5v2c0 .277-.223.5-.5.5s-.5-.223-.5-.5v-2c0-.277.223-.5.5-.5zm-7 0c.277 0 .5.223.5.5v2c0 .277-.223.5-.5.5s-.5-.223-.5-.5v-2c0-.277.223-.5.5-.5zM12 19c1.826 0 3.106-.533 3.865-1.09.758-.555 1.074-1.173 1.074-1.173.33-.605-.618-1.04-.884-.443 0 0-.175.376-.77.813-.598.438-1.648.893-3.286.893-1.64 0-2.614-.45-3.244-.89-.63-.437-.83-.818-.83-.818-.363-.65-1.168-.06-.863.487 0 0 .347.603 1.132 1.15C8.98 18.473 10.175 19 12 19zm0-19C5.36 0 0 5.36 0 12s5.36 12 12 12 12-5.36 12-12S18.64 0 12 0zm0 1c6.103 0 11 4.897 11 11s-4.897 11-11 11S1 18.103 1 12 5.897 1 12 1z"/>
    </svg>
    <p class="footer-smart__text">Помощь</p>
  </a>
  <a href="#" class="footer-smart__item footer-smart__item_more">
    <svg class="footer-smart__icon" width="24" height="24" viewBox="0 0 24 24">
      <path d="M12 9c-1.1 0-2 .9-2 2v7c0 1.1.9 2 2 2s2-.9 2-2v-7c0-1.1-.9-2-2-2zm0 1c.563 0 1 .437 1 1v7c0 .563-.437 1-1 1s-1-.437-1-1v-7c0-.563.437-1 1-1zm0-6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 1c.558 0 1 .442 1 1s-.442 1-1 1-1-.442-1-1 .442-1 1-1zm0-5C5.38 0 0 5.38 0 12s5.38 12 12 12 12-5.38 12-12S18.62 0 12 0zm0 1c6.08 0 11 4.92 11 11s-4.92 11-11 11S1 18.08 1 12 5.92 1 12 1z"/>
    </svg>
    <p class="footer-smart__text">Еще</p>
  </a>
</footer>